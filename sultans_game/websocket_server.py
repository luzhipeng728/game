...
"""
    ...
    async def _call_crewai_agent(self, agent, context: str) -> str:
        ...
        try:
            ...
            print(f"🤖 开始调用智能体: {agent_type}")
            
            agent_instance = agent.get_agent_instance()
            print(f"✅ 成功获取智能体实例: {type(agent_instance).__name__}")
            
            # 🔥 新增：检查LLM配置
            if hasattr(agent, 'llm'):
                llm = agent.llm
                print(f"🧠 LLM配置检查:")
                print(f"   模型名称: {getattr(llm, 'model_name', getattr(llm, 'model', 'unknown'))}")
                print(f"   基础URL: {getattr(llm, 'base_url', getattr(llm, 'openai_api_base', 'unknown'))}")
                print(f"   API Key: {'已设置' if getattr(llm, 'openai_api_key', None) else '未设置'}")
                print(f"   温度: {getattr(llm, 'temperature', 'unknown')}")
                ...
                try:
                    ...
                    test_response = await asyncio.get_event_loop().run_in_executor(
                        None, 
                        lambda: llm.invoke("请简单回答：你好")
                    )
                    print(f"✅ LLM测试成功: {str(test_response)[:50]}...")
                except Exception as test_e:
                    print(f"❌ LLM测试失败: {type(test_e).__name__}: {test_e}")
                    # 如果连基本的LLM调用都失败，直接抛出异常
                    raise Exception(f"LLM配置有问题: {test_e}")
...
"""
            ...
            # 创建单一任务
            print(f"📋 创建任务，提示长度: {len(task_prompt)} 字符")
            task = Task(
            ...
            def run_crew():
                try:
                    print(f"🚀 开始执行Crew.kickoff()...")
                    result = crew.kickoff()
                    print(f"✅ Crew.kickoff()执行完成，结果类型: {type(result)}")
                    
                    if hasattr(result, 'raw'):
                        # 如果是CrewAI的TaskOutput对象，取raw内容
                        response_text = str(result.raw).strip()
                        print(f"📝 从result.raw获取响应: '{response_text[:100]}...'")
                    else:
                        response_text = str(result).strip()
                        print(f"📝 从result直接获取响应: '{response_text[:100]}...'")
